...
tripIDskiplist = ['2023-09-06-18-04-us-ca/sm-s908b','2023-09-07-22-47-us-ca-routebc2/pixel6pro',
                  '2022-04-01-18-22-us-ca-lax-t/pixel6pro'] # 误差太大
    ...
    if starting_flag:
        if tripID not in errortrajlist:
            if tripID not in tripIDskiplist:
                ...
                raw_acc_enu = pd.read_csv(f'{dirname}/Raw_measurement_acc_enu.csv')
                ...
                raw_acc_enu_dic[tripID] = raw_acc_enu
...
#%% raw data saving
with open(dir_path + 'envRLKF/raw_baseline_IMU_multic_enu.pkl', 'wb') as value_file:
...
with open(dir_path + 'envRLKF/raw_tripID_IMU.pkl', 'wb') as value_file:
    pickle.dump(tripIDlist, value_file, True)
value_file.close()
### record for AKF-RL
# record conv velocity and position
with open(dir_path + 'envRLKF/raw_acc_enu.pkl', 'wb') as value_file:
    pickle.dump(raw_acc_enu_dic, value_file, True)
value_file.close()
with open(dir_path + 'envRLKF/raw_rotation_matrix_dic.pkl', 'wb') as value_file:
...
tripID_df.to_csv(dir_path + 'envRLKF/raw_tripID_IMU_enu.csv', index=True)
if processing_gnss:
