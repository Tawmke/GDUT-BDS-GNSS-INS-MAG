...
class LOSPRRprocess():
    ...
    def LOSPRRprocesses(self):
        ...
        for idx, dframe in tqdm(data.groupby("millisSinceGpsEpoch")):
            ...
            dframe_multic.loc[:, 'VE_wls'] = baseline_slice['VE_wls'].to_numpy() * shaped_ones
            dframe_multic.loc[:, 'VN_wls'] = baseline_slice['VN_wls'].to_numpy() * shaped_ones
            dframe_multic.loc[:, 'VU_wls'] = baseline_slice['VU_wls'].to_numpy() * shaped_ones
            dframe_multic.loc[:, 'Acc_e'] = baseline_slice['Acc_e'].to_numpy() * shaped_ones
            dframe_multic.loc[:, 'Acc_n'] = baseline_slice['Acc_n'].to_numpy() * shaped_ones
            dframe_multic.loc[:, 'Acc_u'] = baseline_slice['Acc_u'].to_numpy() * shaped_ones
            dframe_multic.loc[:, 'E_gt'] = baseline_slice['E_gt'].to_numpy() * shaped_ones
            dframe_multic.loc[:, 'N_gt'] = baseline_slice['N_gt'].to_numpy() * shaped_ones
            dframe_multic.loc[:, 'U_gt'] = baseline_slice['U_gt'].to_numpy() * shaped_ones
            dframe_multic.loc[:, 'E_bl'] = baseline_slice['E_bl'].to_numpy() * shaped_ones
            dframe_multic.loc[:, 'N_bl'] = baseline_slice['N_bl'].to_numpy() * shaped_ones
            dframe_multic.loc[:, 'U_bl'] = baseline_slice['U_bl'].to_numpy() * shaped_ones
            dframe_multic.loc[:, 'E_wls'] = baseline_slice['E_wls'].to_numpy() * shaped_ones
            dframe_multic.loc[:, 'N_wls'] = baseline_slice['N_wls'].to_numpy() * shaped_ones
            dframe_multic.loc[:, 'U_wls'] = baseline_slice['U_wls'].to_numpy() * shaped_ones
            dframe_multic.loc[:, 'E_kf'] = baseline_slice['E_kf'].to_numpy() * shaped_ones
            dframe_multic.loc[:, 'N_kf'] = baseline_slice['N_kf'].to_numpy() * shaped_ones
            dframe_multic.loc[:, 'U_kf'] = baseline_slice['U_kf'].to_numpy() * shaped_ones
            dframe_multic.loc[:, 'E_eskf_hfv2'] = baseline_slice['E_eskf_hfv2'].to_numpy() * shaped_ones
            dframe_multic.loc[:, 'N_eskf_hfv2'] = baseline_slice['N_eskf_hfv2'].to_numpy() * shaped_ones
            dframe_multic.loc[:, 'U_eskf_hfv2'] = baseline_slice['U_eskf_hfv2'].to_numpy() * shaped_ones
            try:
                dframe_multic.loc[:, 'E_eskf_hfall'] = baseline_slice['E_eskf_hfall'].to_numpy() * shaped_ones
                dframe_multic.loc[:, 'N_eskf_hfall'] = baseline_slice['N_eskf_hfall'].to_numpy() * shaped_ones
                dframe_multic.loc[:, 'U_eskf_hfall'] = baseline_slice['U_eskf_hfall'].to_numpy() * shaped_ones
            except:
                pass
