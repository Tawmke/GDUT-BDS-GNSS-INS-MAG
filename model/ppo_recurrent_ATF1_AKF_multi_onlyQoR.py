...
from model.common.buffers_AKF_multi import RecurrentRolloutBuffer_AKF_multi
...
class RecurrentPPO(OnPolicyAlgorithm):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            policy_kwargs=policy_kwargs_dic['correct_policy'],
            verbose=verbose,
            seed=seed,
            device=device,
        ...
        self._last_lstm_states_correct = None
        ...
        if 'ATF_trig' in self.policy_kwargs:
            ...
            policy_kwargs_dic['correct_policy'].pop('ATF_trig')
    ...
    def _setup_model(self) -> None:
        ...
        actionspace_correction = spaces.Box(low=-100, high=100, shape=(1, 3), dtype=np.float)
        actionspace_R = spaces.Box(low=-100, high=100, shape=(1, 9), dtype=np.float)
        # define policy Q
        self.policy_correct = self.policy_class(
            self.observation_space,
            actionspace_correction,
            self.lr_schedule,
            use_sde=self.use_sde,
            **self.policy_kwargs,  # pytype:disable=not-instantiable
        )
        self.policy_correct = self.policy_correct.to(self.device)

        lstm_correct = self.policy_correct.lstm_actor

        if not isinstance(self.policy_correct, RecurrentActorCriticPolicy):
            raise ValueError("Policy must subclass RecurrentActorCriticPolicy")

        single_hidden_state_shape = (lstm_correct.num_layers, self.n_envs, lstm_correct.hidden_size)
        # hidden and cell states for actor and critic
        self._last_lstm_states_correct = RNNStates(
            (
        ...
        self.scheduler_correct = th.optim.lr_scheduler.StepLR(self.policy_correct.optimizer, step_size=5e4, gamma=0.99)
        self.scheduler_R = th.optim.lr_scheduler.StepLR(self.policy_R.optimizer, step_size=1e4, gamma=0.9)
        ...
        hidden_state_buffer_shape = (self.n_steps, lstm_correct.num_layers, self.n_envs, lstm_correct.hidden_size)
    ...
    def collect_rollouts(
        ...
        self.policy_correct.set_training_mode(False)
        ...
        if self.use_sde:
            self.policy_correct.reset_noise(env.num_envs)
            self.policy_R.reset_noise(env.num_envs)

        callback.on_rollout_start()

        lstm_states_correct = deepcopy(self._last_lstm_states_correct)
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                ...
                actions_correct, values_correct , log_probs_correct, lstm_states_correct = self.policy_correct.forward(obs_tensor, lstm_states_correct, episode_starts)
                actions_R, values_R, log_probs_R, lstm_states_R = self.policy_R.forward(obs_tensor, lstm_states_R, episode_starts)
                actions = th.cat((actions_correct, actions_R), dim=1)
            actions = actions.cpu().numpy()
            values = (values_correct + values_R) * 0.5
            log_probs = (log_probs_correct + log_probs_R) * 0.5 # how to concat policy logprob ?
            ...
            for idx, done_ in enumerate(dones):
                if (
                    ...
                    terminal_obs = self.policy_correct.obs_to_tensor(infos[idx]["terminal_observation"])[0]
                    with th.no_grad():
                        terminal_lstm_state = (
                            lstm_states_correct.vf[0][:, idx : idx + 1, :],
                            lstm_states_correct.vf[1][:, idx : idx + 1, :],
                        )
                        # terminal_lstm_state = None
                        episode_starts = th.tensor([False]).float().to(self.device)
                        terminal_value_correct = self.policy_correct.predict_values(terminal_obs, terminal_lstm_state, episode_starts)[0]

                    rewards[idx] += self.gamma * (terminal_value_correct + terminal_value_R) * 0.5
            ...
            rollout_buffer.add(
                ...
                values,
                log_probs,
                lstm_states_Q=self._last_lstm_states_correct, # in fact it state for correct policy
                lstm_states_R=self._last_lstm_states_R,
            )

            self._last_obs = new_obs
            self._last_episode_starts = dones
            self._last_lstm_states_correct = lstm_states_correct
            self._last_lstm_states_R = lstm_states_R

        with th.no_grad():
            # Compute value for the last timestep
            episode_starts = th.tensor(dones).float().to(self.device)
            values_correct = self.policy_correct.predict_values(obs_as_tensor(new_obs, self.device), lstm_states_correct.vf, episode_starts)
    ...
    def train(self) -> None:
        ...
        self.policy_correct.set_training_mode(True)
        self.policy_R.set_training_mode(True)
        # Update optimizer learning rate
        # self._update_learning_rate(self.policy_correct.optimizer)
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                if self.use_sde:
                    self.policy_correct.reset_noise(self.batch_size)
                    self.policy_R.reset_noise(self.batch_size)

                values_correct, log_prob_correct, entropy_correct = self.policy_correct.evaluate_actions(
                ...
                values = (values_correct + values_R) * 0.5
                log_prob = (log_prob_correct + log_prob_R) * 0.5
                entropy = (entropy_correct + entropy_R) * 0.5
                ...
                self.logger.record("train/learning_rate", self.policy_correct.optimizer.state_dict()['param_groups'][0]['lr'])
                ...
                #     continue
                self.policy_correct.optimizer.zero_grad()
                loss.backward(retain_graph=True)
                # Clip grad norm
                th.nn.utils.clip_grad_norm_(self.policy_correct.parameters(), self.max_grad_norm)
                self.policy_correct.optimizer.step()
        ...
        if hasattr(self.policy_correct, "log_std"):
            self.logger.record("train/std_correct", th.exp(self.policy_correct.log_std).mean().item())
    ...
    def predict(
        ...
        """
        action_correct, _states = self.policy_correct.predict(observation, state, episode_start, deterministic)
        action_R, _states = self.policy_R.predict(observation, state, episode_start, deterministic)
        action = np.concatenate((action_correct,action_R),axis=1)
    ...
    def _get_torch_save_params(self) -> Tuple[List[str], List[str]]:
        state_dicts = ["policy_correct", "policy_correct.optimizer", "policy_R", "policy_R.optimizer",]
        return state_dicts, []
