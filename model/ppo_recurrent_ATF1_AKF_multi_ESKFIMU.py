...
class RecurrentPPO(OnPolicyAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        actionspace_Q = spaces.Box(low=-100, high=100, shape=(1, 24), dtype=np.float)
        actionspace_R = spaces.Box(low=-100, high=100, shape=(1, 12), dtype=np.float)
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                actions_Q, actions_R  = actions[:,:24], actions[:,-12:]
    ...
    def predict(
        ...
        state_Q: Optional[Tuple[np.ndarray, ...]] = None,
        state_R: Optional[Tuple[np.ndarray, ...]] = None,
        ...
        """
        action_Q, _states_Q = self.policy_Q.predict(observation, state_Q, episode_start, deterministic)
        action_R, _states_R = self.policy_R.predict(observation, state_R, episode_start, deterministic)
        action = np.concatenate((action_Q,action_R),axis=1)
        return action, _states_Q, _states_R
