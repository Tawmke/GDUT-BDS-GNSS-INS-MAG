...
class RecurrentActorCriticPolicy_RSP(ActorCriticPolicy):
    """
    ...
    def __init__(
        ...
        # Setup optimizer with initial learning rate only for ac network
        ### no detach optimizer
        if detach == 'nodetach':
            self.optimizer = self.optimizer_class(self.parameters(), lr=lr_schedule(1), **self.optimizer_kwargs)
        elif detach == 'detach':
        # detach optimizer
            mlp_extractor_parameters = self.mlp_extractor.parameters()
            actionnetwork_parameters = self.action_net.parameters()
            valuenetwork_parameters = self.value_net.parameters()
            allparameters_AC = list(mlp_extractor_parameters)+list(actionnetwork_parameters)+list(valuenetwork_parameters)
            self.optimizer = self.optimizer_class(allparameters_AC, lr=lr_schedule(1), **self.optimizer_kwargs)
