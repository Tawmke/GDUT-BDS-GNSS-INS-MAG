...
class RecurrentPPO_SODA(OnPolicyAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        # 定义soda的结构，其中共享前面的lstm结构
        soda_encoder = DAmodules.EncoderSODA(lstm,DAmodules.SODAMLP(self.policy.lstm_output_dim,
                                                                    self.args.projection_dim, self.args.projection_dim),
                                                                    self.observation_space)
        self.predictor = DAmodules.SODAPredictor(soda_encoder, self.args.projection_dim).cuda()
        self.predictor_target = deepcopy(self.predictor)
        self.soda_optimizer = th.optim.Adam(self.predictor.parameters(), lr=self.args.aux_lr, betas=(self.args.aux_beta, 0.999))
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                if iteration % self.args.aux_update_freq == 0:
                    ...
                    aug_obs = obs.clone()
                    #数据增强方式1：添加高斯白噪声
                    sigma = sigma_compute(aug_obs,SNRdB=self.SNR)
    ...
    def compute_soda_loss(self, x0, x1, lstm_state, episode_starts):
        h0 = self.predictor(x0,lstm_state,episode_starts)
        with th.no_grad():
            h1 = self.predictor_target.encoder(x1,lstm_state,episode_starts)  # 只使用这里的encoder
