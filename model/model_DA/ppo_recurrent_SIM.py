...
import model.model_DA.modulesDA as DAmodules


class RecurrentPPO_SIM(OnPolicyAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        self.policy = self.policy_class(
            ...
            shared_lstm = True, #actor和critic共享lstm模块
        ...
        # 定义sim的结构，其中共享前面的lstm结构，注意critic和actor的lstm结构是分离的
        self.sim_encoder = DAmodules.EncoderSODA(lstm,DAmodules.SODAMLP(self.policy.lstm_output_dim,
                                                                    self.args.projection_dim, self.args.projection_dim),
                                                                    self.observation_space).cuda()
        #self.predictor = DAmodules.SODAPredictor(sim_encoder, self.args.projection_dim).cuda()
        self.sim_optimizer = th.optim.Adam(self.sim_encoder.parameters(), lr=self.args.aux_lr, betas=(self.args.aux_beta, 0.999))
    ...
    def collect_rollouts(
        ...
        self.policy.set_training_mode(False) #设置为测试模式
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                ...
                episode_starts = th.tensor(self._last_episode_starts).float().to(self.device) #对应变量转换为tensor
    ...
    def train(self,iteration) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                self.logger.record('values/perepoch_returns_soda', th.mean(rollout_data.returns[mask]).item())
                ...
                if entropy is None:
                    ...
                    entropy_loss = -th.mean(-log_prob[mask]) # 熵 H
                else:
                ...
                # 以一定的频率更新soda 20230419
                if iteration % self.args.aux_update_freq == 0:
                    self.sim_encoder.train(True)
                    obs = rollout_data.observations
                    lstm_state = rollout_data.lstm_states
                    episode_starts = rollout_data.episode_starts
                    aug_obs = obs.clone()
                    #数据增强方式1：添加高斯白噪声
                    if self.args.Augment=='GN':
                        sigma = sigma_compute(aug_obs,SNRdB=self.args.SNR)
                        aug_obs = aug_obs + sigma * th.randn_like(aug_obs)

                    soda_loss = self.compute_soda_loss(aug_obs, obs, lstm_state, episode_starts )
                    self.soda_optimizer.zero_grad()
                    soda_loss.backward()
                    self.soda_optimizer.step()
                    soft_update_params(self.predictor, self.predictor_target,self.args.soda_tau)
                    self.logger.record('values/loss_soda', th.mean(soda_loss).item())
    ...
    def compute_soda_loss(self, x0, x1, lstm_state, episode_starts):
        h0 = self.sim_encoder(x0,lstm_state,episode_starts)
        h1 = self.sim_encoder(x1,lstm_state,episode_starts)  # 只使用这里的encoder
        h0 = F.normalize(h0, p=2, dim=1)
        h1 = F.normalize(h1, p=2, dim=1)
        loss =  self_supervised_loss(h0,h1)
...
def sigma_compute(train_data,SNRdB= 40):
    ...
    train_data = np.array(train_data.cpu())
...
def soft_update_params(net, target_net, tau): #讲权重复制
    for param, target_param in zip(net.parameters(), target_net.parameters()):
        target_param.data.copy_(
            tau * param.data + (1 - tau) * target_param.data)

def self_supervised_loss(h0,h1):
    batchsize,dim = h0.shape
