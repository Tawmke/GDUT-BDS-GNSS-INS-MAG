...
class RecurrentPPO_RAD(OnPolicyAlgorithm):
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                ...
                if self.args.Augment == 'GN':
                    ...
                    self._last_obs = self._last_obs + np.random.normal(mean, std, len(self._last_obs)) * sigma * mask
                elif self.args.Augment == 'RAS-S':
                    random_number_1 = np.random.uniform(self.args.min, self.args.max)
                    #random_number_2 = np.random.uniform(self.args.min, self.args.max)
                    self._last_obs = self._last_obs * random_number_1
