...
class RecurrentPPO(OnPolicyAlgorithm):
    ...
    def __init__(
        ...
        # generate data for DLRL-log
        # self.obs_data = np.zeros([10000,52])
        # self.Y_data = np.zeros([10000,1])
        # self.idx = 0
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                #     obs = rollout_data.observations[ii,:,:]
                #     Y = rollout_data.returns[ii]
                #     obs = obs.view(-1)
                #     if (th.sum(obs) != 0) and (Y > -5):
                #         self.obs_data[self.idx,:] = obs.cpu().numpy()
                #         self.Y_data[self.idx, :] = Y.cpu().numpy()
                #         self.idx += 1
                #         print(f'{self.idx},{Y}')
                #         if self.idx == 10000:
                #             break
        ...
        # data for DLRL-log
        # if self.idx == 10000:
        #     with open(self.tensorboard_log + f'/trainX_data.pkl', 'wb') as value_file:
        #         pickle.dump(self.obs_data, value_file, True)
        #     value_file.close()
        #
        #     with open(self.tensorboard_log + f'/trainY_data.pkl', 'wb') as value_file:
        #         pickle.dump(self.Y_data, value_file, True)
        #     value_file.close()
