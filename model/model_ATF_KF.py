...
class CustomATF1_AKFRL_processQ(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 256): #注意力自适应权重
        super(CustomATF1_AKFRL_processQ, self).__init__(observation_space, features_dim)
        dim_input1 = observation_space["pos"].shape[-1]
        dim_input2 = observation_space["Q_noise"].shape[-1]
        self.attention1 = Attention1(dim_input1)
        self.attention2 = Attention1(dim_input2)

    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        w1 = self.attention1(observations['pos'])
        w2 = self.attention2(observations['Q_noise'])
        out=torch.cat((observations['pos']*w1,observations['Q_noise']*w2),dim=-1)
        return out

class CustomATF1_AKFRL_measurementR(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 256): #注意力自适应权重
        super(CustomATF1_AKFRL_measurementR, self).__init__(observation_space, features_dim)
        dim_input1 = observation_space["gnss"].shape[-1]
        dim_input2 = observation_space["pos"].shape[-1]
        dim_input3 = observation_space["R_noise"].shape[-1]
        self.attention1 = Attention1(dim_input1)
        self.attention2 = Attention1(dim_input2)
        self.attention3 = Attention1(dim_input3)

    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        w1 = self.attention1(observations['gnss'])
        w2 = self.attention2(observations['pos'])
        w3 = self.attention3(observations['R_noise'])
        out=torch.cat((observations['gnss']*w1,observations['pos']*w2,observations['R_noise']*w3),dim=-1)
        return out

class CustomATF1_AKFRL_loscovR(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 256): #注意力自适应权重
        super(CustomATF1_AKFRL_loscovR, self).__init__(observation_space, features_dim)
        dim_input1 = observation_space["gnss"].shape[-1]
        dim_input3 = observation_space["R_noise"].shape[-1]
        self.attention1 = Attention1(dim_input1)
        self.attention3 = Attention1(dim_input3)
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        w1 = self.attention1(observations['gnss'])
        w3 = self.attention3(observations['R_noise'])
        out=torch.cat((observations['gnss']*w1,observations['R_noise']*w3),dim=-1)
        return out

class CustomATF1_AKFRL_poscovQ(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 256): #注意力自适应权重
        super(CustomATF1_AKFRL_poscovQ, self).__init__(observation_space, features_dim)
        dim_input1 = observation_space["pos"].shape[-1]
        dim_input2 = observation_space["Q_noise"].shape[-1]
        self.attention1 = Attention1(dim_input1)
        self.attention2 = Attention1(dim_input2)
...
class CustomATF1_AKFRL_losposcovQ(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 256): #注意力自适应权重
        super(CustomATF1_AKFRL_losposcovQ, self).__init__(observation_space, features_dim)
        dim_input1 = observation_space["pos"].shape[-1]
        dim_input2 = observation_space["Q_noise"].shape[-1]
        dim_input3 = observation_space["gnss"].shape[-1]
        self.attention1 = Attention1(dim_input1)
        self.attention2 = Attention1(dim_input2)
        self.attention3 = Attention1(dim_input3)
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        w1 = self.attention1(observations['pos'])
        w2 = self.attention2(observations['Q_noise'])
        w3 = self.attention3(observations['gnss'])
        out=torch.cat((observations['pos']*w1,observations['Q_noise']*w2,observations['gnss']*w3),dim=-1)
        return out

class CustomATF1_AKFRL_losposcovR(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 256): #注意力自适应权重
        super(CustomATF1_AKFRL_losposcovR, self).__init__(observation_space, features_dim)
        dim_input1 = observation_space["gnss"].shape[-1]
        dim_input2 = observation_space["pos"].shape[-1]
        dim_input3 = observation_space["R_noise"].shape[-1]
        self.attention1 = Attention1(dim_input1)
        self.attention2 = Attention1(dim_input2)
        self.attention4 = Attention1(dim_input3)

    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        w1 = self.attention1(observations['gnss'])
        w2 = self.attention2(observations['pos'])
        w4 = self.attention4(observations['R_noise'])
        out=torch.cat((observations['gnss']*w1,observations['pos']*w2, observations['R_noise']*w4),dim=-1)
        return out

class CustomATF1_AKFRL_losposcovRinnovation(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 256): #注意力自适应权重
        super(CustomATF1_AKFRL_losposcovRinnovation, self).__init__(observation_space, features_dim)
        dim_input1 = observation_space["gnss"].shape[-1]
        dim_input2 = observation_space["pos"].shape[-1]
        dim_input3 = observation_space["R_noise"].shape[-1]
        dim_input4 = observation_space["innovation"].shape[-1]
        self.attention1 = Attention1(dim_input1)
        self.attention2 = Attention1(dim_input2)
        self.attention3 = Attention1(dim_input3)
        self.attention4 = Attention1(dim_input4)

    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        w1 = self.attention1(observations['gnss'])
        w2 = self.attention2(observations['pos'])
        w3 = self.attention3(observations['R_noise'])
        w4 = self.attention4(observations['innovation'])
        out=torch.cat((observations['gnss']*w1,observations['pos']*w2, observations['R_noise']*w3,
                       observations['innovation']*w4),dim=-1)
        return out

class CustomATF1_AKFRL_losposcovQRinnovation(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 256): #注意力自适应权重
        super(CustomATF1_AKFRL_losposcovQRinnovation, self).__init__(observation_space, features_dim)
        dim_input1 = observation_space["gnss"].shape[-1]
        dim_input2 = observation_space["pos"].shape[-1]
        dim_input3 = observation_space["R_noise"].shape[-1]
        dim_input4 = observation_space["innovation"].shape[-1]
        dim_input5 = observation_space["Q_noise"].shape[-1]
        self.attention1 = Attention1(dim_input1)
        self.attention2 = Attention1(dim_input2)
        self.attention3 = Attention1(dim_input3)
        self.attention4 = Attention1(dim_input4)
        self.attention5 = Attention1(dim_input5)
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        ...
        w3 = self.attention3(observations['R_noise'])
        w4 = self.attention4(observations['innovation'])
        w5 = self.attention5(observations['Q_noise'])
        out=torch.cat((observations['gnss']*w1,observations['pos']*w2, observations['R_noise']*w3,
                       observations['innovation']*w4,observations['Q_noise']*w5),dim=-1)
        return out

class CustomATF1_AKFRL_poscovQinnovation(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 256): #注意力自适应权重
        super(CustomATF1_AKFRL_poscovQinnovation, self).__init__(observation_space, features_dim)
        dim_input1 = observation_space["pos"].shape[-1]
        dim_input2 = observation_space["Q_noise"].shape[-1]
        dim_input3 = observation_space["innovation"].shape[-1]
        self.attention1 = Attention1(dim_input1)
        self.attention2 = Attention1(dim_input2)
        self.attention3 = Attention1(dim_input3)
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        w1 = self.attention1(observations['pos'])
        w2 = self.attention2(observations['Q_noise'])
        w3 = self.attention3(observations['innovation'])
        out=torch.cat((observations['pos']*w1, observations['Q_noise']*w2,observations['innovation']*w3),dim=-1)
        return out

class CustomATF1_AKFRL(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 256): #注意力自适应权重
        super(CustomATF1_AKFRL, self).__init__(observation_space, features_dim)
        dim_input1 = observation_space["gnss"].shape[-1]
        dim_input2 = observation_space["pos"].shape[-1]
        dim_input3 = observation_space["Q_noise"].shape[-1]
        dim_input4 = observation_space["R_noise"].shape[-1]
        self.attention1 = Attention1(dim_input1)
        self.attention2 = Attention1(dim_input2)
        self.attention3 = Attention1(dim_input3)
        self.attention4 = Attention1(dim_input4)
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        w1 = self.attention1(observations['gnss'])
        w2 = self.attention2(observations['pos'])
        w3 = self.attention3(observations['Q_noise'])
        w4 = self.attention4(observations['R_noise'])
        out=torch.cat((observations['gnss']*w1,observations['pos']*w2,
                       observations['Q_noise']*w3,observations['R_noise']*w4),dim=-1)
        return out
