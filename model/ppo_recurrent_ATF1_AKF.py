...
class RecurrentPPO(OnPolicyAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        self.scheduler = th.optim.lr_scheduler.StepLR(self.policy.optimizer, step_size=1e5, gamma=0.9) # step_size=5e4
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                ...
                # record interference time per step
                starttime = time.perf_counter()
                self.policy.get_distribution(obs_tensor, lstm_states.pi, episode_starts)
            ...
            try:
                self.logger.record("values/tripID", infos[0]['tripid'])
            except:
                pass
        ...
        meantime = np.nanmean(time_record)
        stdtime = np.nanstd(time_record)
        print(f'time processing each step:{np.nanmean(time_record):.6f}+{np.nanstd(time_record):.6f}')
        return True
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                self.logger.record("train/learning_rate", self.policy.optimizer.state_dict()['param_groups'][0]['lr'])
