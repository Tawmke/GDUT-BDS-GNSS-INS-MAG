...
def cal_distance_ecef_RLKF(row,baseline_mod):
    ...
    result1 = np.sqrt(((x2 - x1) ** 2 + (y2 - y1) ** 2 + (z2 - z1) ** 2))
    result2 = np.sqrt(((x3 - x1) ** 2 + (y3 - y1) ** 2 + (z3 - z1) ** 2))
    llerr2 = np.sqrt(((x3 - x1) ** 2 + (y3 - y1) ** 2 ))
    herr2 = (z3 - z1)
    herrabs2 = np.abs(z3 - z1)
    if np.isnan(x2) or np.isnan(y2) or np.isnan(z2):
        xerr1 = np.nan
        yerr1 = np.nan
        zerr1 = np.nan
        ...
        yerr1 = np.sqrt(((y2 - y1) ** 2))
        zerr1 = np.sqrt(((z2 - z1) ** 2))
        llerr1 = np.sqrt(((x2 - x1) ** 2 + (y2 - y1) ** 2 ))
        herr1 = (z2 - z1)
        herrabs1 = np.abs(z2 - z1)
    xerr2 = np.sqrt(((x3 - x1) ** 2))
    yerr2 = np.sqrt(((y3 - y1) ** 2))
    zerr2 = np.sqrt(((z3 - z1) ** 2))
...
def recording_results_ecef_RL4KF(dir_path, data_truth_dic,trajdata_range,tripIDlist,logdirname,baseline_mod,traj_record):
    ...
    for train_tripIDnum in range(trajdata_range[0], trajdata_range[1] + 1):
        try:
            pd_train = data_truth_dic[tripIDlist[train_tripIDnum]]
            index_nonull = pd_train['E_RLpredict'].notnull()
            pd_train = pd_train[index_nonull]
            if traj_record:
                # record rl traj
                if baseline_mod == 'kf':
                    record_columns=['UnixTimeMillis','LatitudeDegrees','LongitudeDegrees','AltitudeMeters','E_bl', 'N_bl', 'U_bl',
                                    'E_gt', 'N_gt', 'U_gt','E_kf', 'N_kf', 'U_kf','E_eskf_hfv2', 'N_eskf_hfv2',
                                    'U_eskf_hfv2','E_RLpredict', 'N_RLpredict', 'U_RLpredict']
                elif baseline_mod == 'wls':
                    record_columns = ['UnixTimeMillis', 'ecefX', 'ecefY', 'ecefZ', 'X_RLpredict', 'Y_RLpredict','Z_RLpredict',
                                      'XEcefMeters_wls_igst', 'YEcefMeters_wls_igst', 'ZEcefMeters_wls_igst']
                elif baseline_mod == 'eskf_imumag':
                    record_columns=['UnixTimeMillis','LatitudeDegrees','LongitudeDegrees','AltitudeMeters','E_bl', 'N_bl', 'U_bl',
                                    'E_gt', 'N_gt', 'U_gt','E_kf', 'N_kf', 'U_kf','E_eskf_hfall', 'N_eskf_hfall',
                                    'U_eskf_hfall','E_RLpredict', 'N_RLpredict', 'U_RLpredict']

                pd_record = pd_train[record_columns]
                pd_record.to_csv(logdirname + f'rl_traj_{tripIDlist[train_tripIDnum].replace("/","_")}.csv', index=True)

            if baseline_mod == 'bl':
                test = pd_train.loc[:, ['E_gt', 'N_gt', 'U_gt','E_bl', 'N_bl', 'U_bl','E_eskf_hfv2', 'N_eskf_hfv2', 'U_eskf_hfv2',
                                        'E_RLpredict', 'N_RLpredict', 'U_RLpredict']]
            elif baseline_mod == 'wls_igst' or baseline_mod == 'wls':
                test = pd_train.loc[:, ['E_gt', 'N_gt', 'U_gt','E_wls', 'N_wls', 'U_wls','E_eskf_hfv2', 'N_eskf_hfv2', 'U_eskf_hfv2',
                                        'E_RLpredict', 'N_RLpredict', 'U_RLpredict']]
            elif baseline_mod == 'kf':
                test = pd_train.loc[:, ['E_gt', 'N_gt', 'U_gt','E_kf', 'N_kf', 'U_kf','E_eskf_hfv2', 'N_eskf_hfv2', 'U_eskf_hfv2',
                                        'E_RLpredict', 'N_RLpredict', 'U_RLpredict']]
            elif baseline_mod == 'eskf_imumag':
                test = pd_train.loc[:, ['E_gt', 'N_gt', 'U_gt','E_kf', 'N_kf', 'U_kf','E_eskf_hfall', 'N_eskf_hfall', 'U_eskf_hfall',
                                        'E_RLpredict', 'N_RLpredict', 'U_RLpredict']]
            ...
            test['count_or_ydistance'] = test['or_ydistance'].astype(float)
            test['count_or_zdistance'] = test['or_zdistance'].astype(float)
            rl_lldistance = test.apply(lambda test: cal_distance_ecef_RLKF(test, baseline_mod)[8], axis=1)
            test['rl_lldistance'] = rl_lldistance
            test['rl_hdistance'] = test.apply(lambda test: cal_distance_ecef_RLKF(test, baseline_mod)[9], axis=1)
            test['count_rl_lldistance'] = test['rl_lldistance'].astype(float)
            test['count_rl_hdistance'] = test['rl_hdistance'].astype(float)
            or_lldistance = test.apply(lambda test: cal_distance_ecef_RLKF(test, baseline_mod)[10], axis=1)
            test['or_lldistance'] = or_lldistance
            test['or_hdistance'] = test.apply(lambda test: cal_distance_ecef_RLKF(test, baseline_mod)[11], axis=1)
            test['count_or_lldistance'] = test['or_lldistance'].astype(float)
            test['count_or_hdistance'] = test['or_hdistance'].astype(float)
            test['rl_habsdistance'] = test.apply(lambda test: cal_distance_ecef_RLKF(test, baseline_mod)[12], axis=1)
            test['count_rl_habsdistance'] = test['rl_habsdistance'].astype(float)
            test['or_habsdistance'] = test.apply(lambda test: cal_distance_ecef_RLKF(test, baseline_mod)[13], axis=1)
    ...
    # return avg_xyz_err, avg_rl_llerr, avg_or_llerr